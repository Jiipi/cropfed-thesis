...
from cropfed.data.profiles import MVP_PROFILE_SPECS
...
def data_profile_payload(application_settings: Settings) -> dict[str, Any]:
    ...
    items = [
        _read_profile(data_root / spec.name / "clients" / "partition_summary.json", spec)
...
def _read_profile(path: Path, spec: Any) -> dict[str, Any]:
    base = {
        "name": spec.name,
    ...
    try:
        ...
        clients = _validated_clients(document)
...
def _validated_clients(document: object) -> list[dict[str, Any]]:
    ...
    for source in source_clients:
        ...
        if len(counts) != len(TOMATO_CLASSES) or any(value < 0 for value in counts):
